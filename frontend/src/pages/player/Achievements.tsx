...
import {
  useCatalog, useCatalogText, useTeamWall, overallPoints, overallRank,
  type CatalogEntry, type PlayerAchievements, type EarnedCrest, type GroupProgress, type TeamWall,
} from '../../api/achievements';
import { ACHIEVEMENTS_ORIGIN } from '../../hubOrigin';
...
function CrestCard({ entry, group, onOpen }: {
  ...
  return (
    <button type="button" onClick={() => onOpen(entry.code)} className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col items-center text-center lift cursor-pointer">
      ...
      <p className="text-xs text-gray-400 mt-0.5">{text.description(entry)}</p>
      <div className="w-full mt-3">
        ...
        <p className="text-[11px] text-gray-500 mt-1.5">
          {next === null
            ? t('achievements.maxed', { value, unit: text.unit(entry) })
            : t('achievements.progress', { value, next, unit: text.unit(entry) })}
        </p>
      </div>
    </button>
  );
...
function MyCrests({ userId, onOpen }: { userId: string; onOpen: (code: string) => void }) {
  ...
  const categories: { key: CatalogEntry['category']; label: string }[] = [
    { key: 'performance', label: t('achievements.catPerformance') },
    { key: 'reliability', label: t('achievements.catReliability') },
  ];
  ...
  return (
    <>
      ...
      >
        <div>
          <p className="text-sm font-semibold text-gray-900">{t('achievements.yourProfile')}</p>
          <p className="text-xs text-gray-400 mt-0.5">{t('achievements.yourProfileSub')}</p>
        </div>
        <span className="text-gray-300 group-hover:text-brand-green transition-colors text-lg">→</span>
      </Link>
      <p className="text-xs text-gray-400">
        {t('achievements.seasonNote', { year: data.seasonYear })}
      </p>
      ...
      {lockedCount > 0 && (
        ...
        >
          {showLocked ? t('achievements.hideLocked') : t('achievements.showLocked', { count: lockedCount })}
        </button>
      )}
    </>
  );
...
function TeamWallView({ onOpen }: { onOpen: (code: string) => void }) {
  ...
  return (
    <>
      <p className="text-xs text-gray-400">{t('achievements.wallNote', { year: data.seasonYear })}</p>
        ...
        <section key={tier ?? 'unranked'}>
          <div className="flex items-center gap-2 mb-2 mt-4">
            ...
            <h2 className="text-sm font-bold" style={{ color: tier ? TIER_META[tier].ribbon : '#9ca3af' }}>
              {tier ? TIER_META[tier].label : t('hub.unranked')}
            </h2>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-gray-400 shrink-0 ml-auto">{t('crests.count', { count: p.crests.length })}</span>
                  </div>
      ...
      {buckets.length === 0 && <p className="text-sm text-gray-400 text-center py-8">{t('crests.noneThisSeason')}</p>}

      {bestTeam.size > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-gray-700 mb-1 mt-6">{t('achievements.teamTrophies')}</h2>
          <p className="text-xs text-gray-400 mb-2">{t('achievements.teamTrophiesSub')}</p>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
          {(['mine', 'team'] as const).map(tabKey => (
            ...
            >
              {tabKey === 'mine' ? t('achievements.tabMine') : t('achievements.tabTeam')}
            </button>
          ))}
        </div>

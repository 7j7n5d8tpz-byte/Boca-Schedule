-- Extend merge_placeholder_player to also carry a placeholder's fines across to
-- the real account. (The original function predates the fines history import.)
-- Fines have no per-player uniqueness, so a plain re-point is safe.
